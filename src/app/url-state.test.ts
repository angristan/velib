...
it("round-trips durable replay and map state", () => {
  const state = parseAppUrlState(
    "?station=1001&q=horloge&filter=electric&mode=replay&window=30&at=1784625060&layer=heatmap&basemap=dark&lat=48.85600&lng=2.34200&z=15.25",
  )
...
it("rejects malformed enums, station IDs, and camera coordinates", () => {
  ...
  assert.strictEqual(state.mapBackground, "light")
